---
name: auto-coder
description: Autonomous spec-driven development agent. Syncs DEV_SPEC.md into chapter-based reference files, identifies the next pending task from the schedule, implements code following spec architecture and patterns, runs tests with up to 3 auto-fix rounds, and automatically commits completed work locally with a concise Chinese message. Use when user says "auto code", "自动开发", "自动写代码", "auto dev", "一键开发", "autopilot", or wants fully automated spec-to-code workflow.
---

# Auto Coder

One trigger completes **read spec → find task → code → test → persist progress → local commit**.

Optional modifier: append a task ID (e.g. `auto code B2`) to target a specific task.
...
```
Sync Spec → Find Task → Implement → Test (≤3 fix rounds) → Persist → Local Commit
```

Run the full pipeline autonomously. After tests pass, commit locally without asking for confirmation. Never push automatically.
...
3. **Stage only this cycle's files**: include implementation, tests, `DEV_SPEC.md`, and synced references. Do not stage unrelated user changes.
4. **Commit automatically** after tests pass and progress is synced:
   - Use a short, easy-to-understand Chinese subject.
   - Format: `完成 [任务编号]：任务名称`
   - Example: `完成 [A3]：配置加载与校验`
   - Commit locally only. Never push automatically.
5. **Show summary**:
...
✅ [A3] 配置加载与校验 — done
   ...
   Tests: 8/8 passed
   Commit: 完成 [A3]：配置加载与校验
```

On "next", loop back to step 1 and start the next task. Each completed task gets its own local commit.
